...
public partial class BrowserSignInWindow : Window
{
    ...
    private async void OnNavigationCompleted(object? sender, CoreWebView2NavigationCompletedEventArgs e)
    {
        ...
        try
        {
...
#if DEBUG
            LoggingService.Instance.Log($"[DEBUG][BrowserSignIn] NavigationCompleted — url: {currentUrl}, success: {e.IsSuccess}, ssoAttempted: {_ssoRedirectAttempted}");
#endif
...
#if DEBUG
            var targetSessionKeys = targetCookies.Where(c => c.Name == "sessionKey").Select(c => $"domain={c.Domain}, value={c.Value[..Math.Min(8, c.Value.Length)]}...");
            LoggingService.Instance.Log($"[DEBUG][BrowserSignIn] Step1 target cookies ({_targetUrl}): sessionKeys=[{string.Join(", ", targetSessionKeys)}]");
#endif
...
#if DEBUG
            var currentSessionKeys = currentCookies.Where(c => c.Name == "sessionKey").Select(c => $"domain={c.Domain}, value={c.Value[..Math.Min(8, c.Value.Length)]}...");
            LoggingService.Instance.Log($"[DEBUG][BrowserSignIn] Step2 current cookies ({currentUrl}): sessionKeys=[{string.Join(", ", currentSessionKeys)}]");
#endif
            foreach (var cookie in currentCookies)
            {
                if (cookie.Name == "sessionKey")
                {
                    // User authenticated on a different domain. Navigate to the target login page
                    // to trigger cross-domain SSO, which should set the target domain's cookie.
                    if (!_ssoRedirectAttempted)
                    {
#if DEBUG
                        LoggingService.Instance.Log($"[DEBUG][BrowserSignIn] Step2 SSO redirect — navigating to {_targetUrl}, saving fallback cookie domain: {cookie.Domain}");
#endif
                        _ssoRedirectAttempted = true;
                        _fallbackCookie = cookie;
            ...
            // 3. SSO redirect completed but landed on a page with no sessionKey
            //    (target domain didn't set one and didn't redirect back) — use saved cookie
            if (_ssoRedirectAttempted && _fallbackCookie != null)
            {
#if DEBUG
...
#if DEBUG
            LoggingService.Instance.Log($"[DEBUG][BrowserSignIn] No sessionKey found on any domain — waiting for next navigation");
#endif
        }
        catch (Exception ex)
        {
...
#if DEBUG
        var currentUrl = WebView.CoreWebView2.Source;
        LoggingService.Instance.Log($"[DEBUG][BrowserSignIn] CaptureAndClose — cookie domain: {cookie.Domain}, currentUrl: {currentUrl}, will fetch: {currentUrl}/api/organizations");
#endif
